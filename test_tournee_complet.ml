...
(*      foncteur de tournée, pour la partie 1                             *)
(*                                                                        *)
(**************************************************************************)
...
let rec generate_files_test i =
  let oc1 = open_out ("test_complet/files.txt") in
  let rec generate_files_test_aux i =
    if i < 0 then ()
      ...
      let oc2 = open_out ("test_complet/"^file_name) in
      ...
      let road_list = main_graphe_complet
          ~path_from_root_param:("test_complet/"^param_file_name)
          ~path_from_root_city:("test_complet/"^file_name)
      in
...
let rec check_test i =
  let oc1 = Scanning.from_channel (open_in ("test_complet/files.txt")) in
